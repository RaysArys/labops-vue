...
export class CalibrationParametersAndWadekReport1788957000000
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    `);
    await queryRunner.query(
      `CREATE INDEX "IDX_calibration_parameters_category_active" ON "calibration_parameters" ("kategori_aset", "aktif")`,
    );
    await queryRunner.query(`
    ...
    `);
    await queryRunner.query(
      `ALTER TABLE "calibration_logs" ADD "pengukuran" jsonb NOT NULL DEFAULT '[]'`,
    );
    await queryRunner.query(
      `ALTER TABLE "work_orders" ADD "laporan_ke_wadek" boolean NOT NULL DEFAULT false`,
    );
    await queryRunner.query(
      `ALTER TABLE "work_orders" ADD "waktu_laporan_ke_wadek" TIMESTAMP`,
    );
    await queryRunner.query(
      `ALTER TABLE "work_orders" ADD "dilaporkan_ke_wadek_oleh" character varying`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "work_orders" DROP COLUMN "dilaporkan_ke_wadek_oleh"`,
    );
    await queryRunner.query(
      `ALTER TABLE "work_orders" DROP COLUMN "waktu_laporan_ke_wadek"`,
    );
    await queryRunner.query(
      `ALTER TABLE "work_orders" DROP COLUMN "laporan_ke_wadek"`,
    );
    await queryRunner.query(
      `ALTER TABLE "calibration_logs" DROP COLUMN "pengukuran"`,
    );
